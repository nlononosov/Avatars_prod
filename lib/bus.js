        ...
        sub.on('message', (channel, message) => {
          try {
            ...
            if (parsed.target === 'global') {
              ...
              // Normalize streamerId from Redis message
              dispatchToStreamer(String(parsed.streamerId || ''), parsed.event, parsed.payload, { remote: true });
            }
          } catch (error) {
...
function overlayEventsHandler(req, res) {
  ...
  // Get streamer ID from query parameter and normalize to string
  const streamerId = String(req.query.streamer_id || req.query.streamer || '');
  ...
  // Add to specific streamer channel (normalized to string)
  const normalizedStreamerId = String(streamerId);
  if (!streamerChannels.has(normalizedStreamerId)) {
    streamerChannels.set(normalizedStreamerId, new Set());
    console.log(`[bus] Created new channel for streamer ${normalizedStreamerId}`);
  }
  streamerChannels.get(normalizedStreamerId).add(res);
  console.log(`[bus] Added subscriber for streamer ${normalizedStreamerId}, total: ${streamerChannels.get(normalizedStreamerId).size}`);
  refreshOverlayMetrics();

  req.on('close', () => {
    const normalizedStreamerId = String(streamerId);
    const channel = streamerChannels.get(normalizedStreamerId);
    if (channel) {
      channel.delete(res);
      if (channel.size === 0) {
        streamerChannels.delete(normalizedStreamerId);
        console.log(`[bus] Removed empty channel for streamer ${normalizedStreamerId}`);
      }
    }
    refreshOverlayMetrics();
  });
...
async function emitToStreamer(streamerId, event, payload) {
  // Normalize streamerId to string for consistent key matching
  const normalizedStreamerId = String(streamerId);
  
  await ensureRedisPublisher()
    .then((redis) => redis.publish(STREAM_BROADCAST_CHANNEL, JSON.stringify({ target: 'streamer', streamerId: normalizedStreamerId, event, payload })))
    .catch((error) => console.error('[bus] Failed to publish streamer event to redis', error));

  dispatchToStreamer(normalizedStreamerId, event, payload, { remote: false });
}
...
function dispatchToStreamer(streamerId, event, payload, meta = {}) {
  ...
  const normalizedStreamerId = String(streamerId);
  const data = JSON.stringify(payload || {});
  const streamerSubscribers = streamerChannels.get(normalizedStreamerId);

  if (!streamerSubscribers || streamerSubscribers.size === 0) {
    console.log(`[bus] WARNING: No subscribers for streamer ${normalizedStreamerId} for event "${event}"`);
    console.log(`[bus] Available channels:`, Array.from(streamerChannels.keys()));
    return;
  }
